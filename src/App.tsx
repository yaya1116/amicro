...
export default function App() {
  ...
  return <MotionConfig reducedMotion="user">
    ...
    <header className="site-header">
      ...
      <div className="header-actions"><button className="icon-button" onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} aria-label={theme === 'light' ? '切換深色模式' : '切換淺色模式'}>{theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}</button><a className="header-cta" href="/pricing" onClick={e => navClick(e, '/pricing')}>NT$99 即用包 <ArrowUpRight size={15} /></a><button className="icon-button menu-toggle" aria-label={menuOpen ? '關閉選單' : '開啟選單'} aria-expanded={menuOpen} aria-controls="sidebar" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X /> : <Menu />}</button></div>
    </header>
    <div className="app-layout">
      ...
      <main id="main" className="main-content" tabIndex={-1}>
        ...
        {isCatalog ? <>
          {path === '/' && <Hero browse={() => collectionRef.current?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' })} pricing={() => navigate('/pricing')} />}
